...
The following methods allow for interaction into the Nessus
 `scans </api#/resources/scans>`_ API endpoints.
...
class ScansAPI(NessusEndpoint):
    ...
    def attachment(self, scan_id, attachment_id, key, fobj=None):
        ...
        `scans: attachments </api#/resources/scans/attachments>`_
        ...
        Returns:
            FileObject: A file-like object with the attachment written into it.
    ...
    def configure(self, scan_id, **kw):
        ...
        `scans: configure </api#/resources/scans/configure>`_
        ...
        return self._api.put('scans/{}'.format(
            self._check('scan_id', scan_id, int)),
                             json=scan).json()
    ...
    def copy(self, scan_id, folder_id=None, name=None):
        ...
        `scans: copy </api#/resources/scans/copy>`_
        ...
        return self._api.post('scans/{}/copy'.format(self._check(
            'scan_id', scan_id, int)), json=payload).json()

    def create(self, **kw):
        '''
        Create a new scan.

        `scans: create </api#/resources/scans/create>`_
    ...
    def create_from_details(self, scan):
        '''
        Create a scan from the output of self.details()
        instead of keywords.
        '''
    ...
    def delete(self, *scan_ids):
        ...
        `scans: delete </api#/resources/scans/delete>`_
        ...
        '''
        if len(scan_ids) <= 1:
            self._api.delete('scans/{}'.format(
                self._check('scan_ids', scan_ids[0], int)
            ))
        else:
            self._api.delete('scans',
                json={'ids': [self._check('scan_ids', i, int) for i in scan_ids]})

    def delete_history(self, scan_id, history_id):
        '''
        Remove an instance of a scan from a scan history.

        `scans: delete-history </api#/resources/scans/delete-history>`_
    ...
    def export_formats(self, scan_id):
        '''
        Returns available export formats and report options.

        `scans: export_formats </api#/resources/scans/export-formats>`_

        Args:
            scan_id (int): The unique identifier for the scan.

        Returns:
            dict: the available export and report formatting options.

        Examples:
            Retrieve the export formats:

            >>> formats = nessus.scans.export_formats(1)
        '''

        return self._api.get('scans/{}/export/formats'.format(
            self._check('scan_id', scan_id, int))).json()
    ...
    def results(self, scan_id, history_id=None):
        ...
        `scans: details </api#/resources/scans/details>`_
    ...
    def export(self, scan_id, *filters, **kw):
        ...
        `scans: export </api#/resources/scans/export-request>`_
        ...
        fobj = kw.get('fobj', BytesIO)

        # The first thing that we need to do is make the request and get the
        # File id for the job.
        file_id = self._api.post('scans/{}/export'.format(
            self._check('scan_id', scan_id, int)),
            params=params, json=payload).json()['file']
        self._api._log.debug('Initiated scan export {}'.format(file_id))
    ...
    def host_details(self, scan_id, host_id, history_id=None):
        ...
        `scans: host-details </api#/resources/scans/host-details>`_
    ...
    def import_scan(self, fobj, folder_id=None, password=None, aggregate=None):
        '''
        Import a scan report into Nessus.

        `scans: import </api#/resources/scans/import>`_
        ...
        # Upload the file to the Nessus and store the resulting filename in
        # the payload.
        payload['file'] = self._api.files.upload(fobj)

        # make the call to Nessus to import and then return the result to
        # the caller.
    ...
    def launch(self, scan_id, targets=None):
        ...
        `scans: launch </api#/resources/scans/launch>`_
    ...
    def list(self, folder_id=None, last_modified=None):
        ...
        `scans: list </api#/resources/scans/list>`_
    ...
    def pause(self, scan_id, block=False):
        ...
        `scans: pause </api#/resources/scans/pause>`_
    ...
    def plugin_output(self, scan_id, host_id, plugin_id, history_id=None):
        ...
        `scans: plugin-output </api#/resources/scans/plugin-output>`_
    ...
    def set_read_status(self, scan_id, read_status):
        ...
        `scans: read-status </api#/resources/scans/read-status>`_
    ...
    def resume(self, scan_id):
        ...
        `scans: resume </api#/resources/scans/resume>`_
    ...
    def schedule(self, scan_id, enabled):
        ...
        `scans: schedule </api#/resources/scans/schedule>`_
    ...
    def stop(self, scan_id, block=False):
        ...
        `scans: stop </api#/resources/scans/stop>`_
    ...
    def timezones(self):
        ...
        `scans: timezones </api#/resources/scans/timezones>`_
